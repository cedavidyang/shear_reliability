...
parfor i=1:nCase
    ...
    for i_factor = 1:nFactor
        [Vtotal_design(i_factor),~,~,~] = Vtotal_ACI_design(db_mean, 'W', factor(i_factor));
        for i_LD=1:nLD
...
%             Rparam = [mean(Vs+Vc), std(Vs+Vc)/mean(Vs+Vc), Vs_design+Vc_design];
            ...
            if RE_tmp(i_LD, i_factor)>7
                disp('suspiciously large \beta')
            elseif RE_tmp(i_LD, i_factor)<4
                disp('suspiciously samll \beta')
            end
        end
        RE(:, :, i) = RE_tmp;
    end
