...
load db_Side
load db_U
[Vpre1, ~, ~, ~] = Vtotal_GB(db_Side, 'Side');
[Vpre2, ~, ~, ~] = Vtotal_GB(db_U, 'U');
Vpre = [Vpre1; Vpre2];
Vexp1 = db_Side(:, 21);
Vexp2 = db_U(:, 21);
Vexp = [Vexp1; Vexp2];
...
load db_design_GB
db_design = [db_design_GB; db_design_GB];
...
%     f_c_std = 0.2*f_c_mean;
    ...
    if i<=nCase/2
        [Vtotal, ~, ~, ~] = Vtotal_GB(db_smp, 'Side');
    else
        [Vtotal, ~, ~, ~] = Vtotal_GB(db_smp, 'U');
    end
    ...
    for i_factor = 1:nFactor
        gamma_frp = 1.40;
        if i<=nCase/2
            [Vtotal_design(i_factor),~,~,~] = Vtotal_GB_design(db_mean, 'Side', factor(i_factor), gamma_frp);
        else
            [Vtotal_design(i_factor),~,~,~] = Vtotal_GB_design(db_mean, 'U', factor(i_factor), gamma_frp);
        end
        for i_LD=1:nLD
...
%             Rparam = [mean(Vs+Vc), std(Vs+Vc)/mean(Vs+Vc), Vs_design+Vc_design];
            RE_tmp(i_LD, i_factor) = form_GB(MEparam, LD(i_LD), Rparam );
        end
        RE(:, :, i) = RE_tmp;
    end
end
...
REdata_GB_Side_U = RE;
save('REdata_GB_Side_U_12Frp_detConc.mat', 'REdata_GB_Side_U');
% save('REdata_GB_Side_U_12Frp_125Conc.mat', 'REdata_GB_Side_U');
