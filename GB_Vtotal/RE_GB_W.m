...
% Model Error
load db_W
[Vpre, Vfrp_pre, Vs_pre,  Vc_pre] = Vtotal_GB(db_W, 'W');
% [Vpre2, Vfrp_pre2, Vs_pre2,  Vc_pre2] = Vtotal_HK(db_W, 'W');
Vexp = db_W(:, 21);
...
%     f_c_std = 0.2*f_c_mean;
    ...
    for i_factor = 1:nFactor
        gamma_frpb = 1.00;
        [Vtotal_design(i_factor),~,~,~] = Vtotal_GB_design(db_mean, 'W', gamma_frpb, factor(i_factor));
        for i_LD=1:nLD
...
REdata_GB_W = RE;
save('REdata_GB_W_12Frp_detConc.mat', 'REdata_GB_W');
% save('REdata_GB_W_12Frp_125Conc.mat', 'REdata_GB_W');
