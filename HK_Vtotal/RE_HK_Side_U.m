...
load db_Side
load db_U
[Vpre1, ~, ~, ~] = Vtotal_HK(db_Side, 'Side');
[Vpre2, ~, ~, ~] = Vtotal_HK(db_U, 'U');
Vpre = [Vpre1; Vpre2];
Vexp1 = db_Side(:, 21);
Vexp2 = db_U(:, 21);
Vexp = [Vexp1; Vexp2];
...
load db_design_HK
db_design = [db_design_HK; db_design_HK];
...
%     f_c_std = 0.2*f_c_mean;
    ...
    for i_factor = 1:nFactor
        gamma_frp = 1.40;
        if i<=nCase/2
            [Vtotal_design(i_factor),~,~,~] = Vtotal_HK_design(db_mean, 'Side', factor(i_factor), gamma_frp);
        else
            [Vtotal_design(i_factor),~,~,~] = Vtotal_HK_design(db_mean, 'U', factor(i_factor), gamma_frp);
        end
        for i_LD=1:nLD
...
%             Rparam = [mean(Vs+Vc), std(Vs+Vc)/mean(Vs+Vc), Vs_design+Vc_design];
            RE_tmp(i_LD, i_factor) = form_HK(MEparam, LD(i_LD), Rparam );
        end
        RE(:, :, i) = RE_tmp;
    end
end
...
REdata_HK_Side_U = RE;
save('REdata_HK_Side_U_12Frp_detConc.mat', 'REdata_HK_Side_U');
% save('REdata_HK_Side_U_12Frp_125Conc.mat', 'REdata_HK_Side_U');
